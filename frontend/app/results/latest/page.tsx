...
export default function LatestResultPage() {
  ...
  useEffect(() => {
    getLatestReport()
      .then((data) => {
        ...
        // Redirect to the canonical URL with report ID for bookmarkability
        router.replace(`/results/${data.report_id}`);
  ...
  const handleCheck = async () => {
    if (!report || busy || !reportId) return;
    setBusy("check");
    setNotice("");
    try {
    ...
    <StudentShellClient title="完整报告">
      ...
      {loading ? (
        <SectionCard title="加载中">
          <p style={{ textAlign: "center", padding: "40px", color: "#888" }}>正在加载最新报告...</p>
        </SectionCard>
      ) : error ? (
        <SectionCard title="提示">
          <p style={{ color: "#b91c1c" }}>{error}</p>
          <Link href="/student" className="btn-secondary" style={{ display: "inline-block", marginTop: 12, textDecoration: "none", padding: "8px 14px" }}>
            返回首页
          </Link>
        </SectionCard>
      ) : (
        <>
          ...
          <SectionCard title={`报告 #${report?.report_id ?? "最新"}`}>
